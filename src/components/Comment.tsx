...
interface CommentProps {
  ...
  isReply?: boolean;
  showMoreRepliesCount?: number; // New prop for showing more replies
  onShowMoreReplies?: (commentId: string) => void; // New prop for handling show more replies click
}

export default function Comment({ comment, onLike, onReply, isReply, showMoreRepliesCount, onShowMoreReplies }: CommentProps) {
  const handleLike = () => {
    onLike?.(comment.id);
  };
  ...
  return (
    <>
      ...
      {showMoreRepliesCount && showMoreRepliesCount > 0 && (
        <div
          className="comment-show-more relative flex gap-4 p-6 hover:bg-gray-50/50 transition-colors"
          style={{ '--depth': (comment.depth || 0) + 1 } as React.CSSProperties}
        >
          <div className="comment-avatar flex-shrink-0">
